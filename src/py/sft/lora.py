...
class LoRA_handler:
    def __init__(self):
        ...
        self.raw_dataset = load_dataset('json', data_files=loraDataPath,split='train')
        self.tokenized_dataset = self.tokenize_data(self.raw_dataset)
        ...
        self.model = AutoModelForCausalLM.from_pretrained(self.RAW_MODEL_PATH, device_map=self.device_0, torch_dtype=torch.bfloat16).to(self.device_0)
        
        self.LORA_OUTPUT_DIR = os.path.join(project_root_dir,'outputs','SFT','LoRAForDPO_NABFinalAverageDatas') 
        self.LORA_Logging_OUTPUT_DIR = os.path.join(project_root_dir,'outputs','SFT','LoRAForDPO_NABFinalAverageDatas/logging') 
        ...
        self.training_args = TrainingArguments(
            ...
            save_steps=500,
            logging_steps=100,
            logging_dir=self.LORA_Logging_OUTPUT_DIR,
            learning_rate=2e-4,
    ...
    def preprocess_function(self,examples):
        ...
        myLen = len(examples['input'])
        inputs = [
            f"{examples['instruction'][i]}\n{examples['input'][i]}"
            for i in range(myLen)
        ]
        outputs = [examples['output'][i] for i in range(myLen)]
        # 对输入和输出分别进行分词
    ...
    def tokenize_data(self, dataset) -> Dataset:
        # 使用 map 进行批量处理
