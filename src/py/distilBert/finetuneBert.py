...
class EarlyStoppingCallback(TrainerCallback):
    ...
    def __init__(self, patience=3, min_delta=0.01, train_loss_threshold=0.05, eval_loss_threshold=0.01):
        """
        Args:
            patience (int): 允许的连续不下降次数。
            min_delta (float): 损失值的最小变化量，低于此值视为没有显著下降。
            train_loss_threshold (float): 如果 train_loss 低于此值，则触发早停。
            eval_loss_threshold (float): 如果 eval_loss 低于此值，则触发早停。
        """
    ...
    def on_step_end(self, args, state, control, **kwargs):
        # 在每个训练步骤结束时检查损失
        if state.log_history:  # 确保有日志记录
            logs = state.log_history[-1]
            train_loss = logs.get("loss", None)
            eval_loss = logs.get("eval_loss", None)

            # 更新 train_loss 和 eval_loss
            ...
            # 检查是否满足早停条件
            if self._should_stop(train_loss, eval_loss):
                control.should_training_stop = True

        return control

    def _should_stop(self, train_loss, eval_loss):
        """
        判断是否满足早停条件。
        """
        # 条件 1: train_loss 和 eval_loss 连续 patience 次没有显著下降
        if self.no_improvement_count >= self.patience:
            print("############################################### Early stopping triggered For train_loss&eval_loss stopping change  ###############################################")
            return True

        # 条件 2: train_loss 低于阈值且 eval_loss 低于阈值
        if train_loss is not None and eval_loss is not None:
            if train_loss < self.train_loss_threshold and eval_loss < self.eval_loss_threshold:
                print("############################################### Early stopping triggered For train_loss&eval_loss lower then threshold  ###############################################")
                return True
...
class TensorBoardCallback(TrainerCallback):
    def __init__(self, log_dir=os.path.join(project_root_dir,'outputs','DistilBert','logging','finetuneBert')):
        self.writer = SummaryWriter(log_dir)

    def on_log(self, args, state, control, logs=None, **kwargs):
        if logs:
            for key, value in logs.items():
                if "loss" in key or "accuracy" in key or "eval_loss" in key:
                    self.writer.add_scalar(key, value, state.global_step)

    def on_train_end(self, args, state, control, **kwargs):
        # 训练结束时关闭 TensorBoard 写入器
        self.writer.close()
...
def main():
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if is_regression:
            ...
            result = metric.compute(predictions=preds, references=p.label_ids, average="micro")   # 微平均F1
        else:
    ...
    if data_args.pad_to_max_length:
        ...
        data_collator = DataCollatorWithPadding(tokenizer, pad_to_multiple_of=8)  # 动态tokenize
    else:
        data_collator = None  # Trainer will automate processing 
    ...
    trainer = Trainer(
        ...
        # callbacks=[EarlyStoppingCallback(patience=40), TensorBoardCallback()]
