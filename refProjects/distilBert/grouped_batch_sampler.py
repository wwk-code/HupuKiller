...
'''
lengths: 句子的长度列表
k: 如果 k > 0，则生成一个从 3 开始、步长为 4 的区间列表，直到 k 结束。
例如，k = 20 时，生成的区间列表为 [3, 7, 11, 15, 19]。
如果 k <= 0，则使用默认的区间列表 [10]。
例如，k = 0 时，生成的区间列表为 [10]。

ret:
groups: 一个列表，表示每个 lengths 值所属的长度分组索引
'''
=======

>>>>>>> dbfd4f9d007c188260cf750a554c6901c8a3aa9e
=======
